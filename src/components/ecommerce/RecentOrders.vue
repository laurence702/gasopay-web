  ...
  >
    ...
    <div class="max-w-full overflow-x-auto custom-scrollbar">
      <table class="min-w-full">
        ...
        <tbody>
          <tr v-if="loading">
            <td colspan="4" class="py-6 text-center text-gray-500">Loading recent orders...</td>
          </tr>
          <tr v-else-if="error">
            <td colspan="4" class="py-6 text-center text-red-500">{{ error }}</td>
          </tr>
          <tr v-else-if="products.length === 0">
            <td colspan="4" class="py-6 text-center text-gray-400">No recent orders found.</td>
          </tr>
          <tr
            v-else
          ...
          >
            ...
            <td class="py-3 whitespace-nowrap">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">₦{{ formatAmount(product.amount_due) }}</p>
            </td>
            <td class="py-3 whitespace-nowrap">
              <span
                :class="{
...
const getProductImage = (productName: string) => {
  ...
  if (name.includes('lpg')) {
    return '/images/product/LPG.png';
  } else if (name.includes('pms')) {
    return '/images/product/PMS.png';
  } else if (name.includes('cng')) {
    return '/images/product/CNG2.png';
  } else {
    return '/images/product/product-01.jpg'; // A default image from the public folder
  }
};
...
onMounted(async () => {
  try {
    loading.value = true;
    error.value = null;
    const response = await apiService.get<{ data: Order[] }>(getApiUrl('/orders'), authStore.token || undefined);
    products.value = response.data.slice(0, 5);
    loading.value = false;
  } catch (err) {
    error.value = 'Unable to load recent orders.';
    loading.value = false;
    products.value = [];
  }
});
