...
onMounted(async () => {
  try {
    // Fetch orders
    const response = await apiService.get<any>(getApiUrl('/orders'), authStore.token || undefined);
    ...
    orders.forEach((order: any) => {
      if (order.created_at) {
        ...
        // Sales: sum amount_due for completed payment_status
        if (order.payment_status === 'completed') {
          monthlySales[month] += Number(order.amount_due) || 0;
        }
        // Revenue: sum amount_due for all orders (or adjust as needed)
        monthlyRevenue[month] += Number(order.amount_due) || 0;
      }
    });
    series.value = [
    ...
    if (monthlySales.every((v: number) => v === 0) && monthlyRevenue.every((v: number) => v === 0)) {
